 ...
 */
export function computeLoadScore(metrics) {
  const { wpmScore, pauseScore, backspaceScore, consistencyScore } =
    signalScores(metrics);
...
export function calculateBehaviourState(metrics = {}, previousLevel = null) {
  ...
  const score = computeLoadScore(metrics);
...
export function estimateWorkload(metrics, previousLevel = null) {
  return calculateBehaviourState(metrics, previousLevel);
}
